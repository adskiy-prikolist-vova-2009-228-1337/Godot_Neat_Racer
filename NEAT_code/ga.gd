...
func update_curr_species() -> Array:
	...
	if updated_species.size() == 0:
		push_error("update_spec size is zero"); breakpoint
	if total_adjusted_species_avg_fitness == 0:
		push_error("fitness is 0 (no fitness, how would we learn?)"); breakpoint
